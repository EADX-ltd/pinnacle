...
| Current Phase | `3` |
| Last Updated (UTC) | `2026-04-02 13:08` |
| Updated By | `agent` |
| Notes | `Text draft handoff now commits the current note and starts the next one on the next main-loop turn, explicitly tears down the old AppKit editor, keeps the overlay panel key during text-to-text handoffs, and routes text-editor-originated clicks via screen coordinates so a click outside the current field still starts the next text draft; remaining active blocker is still the manual macOS GUI checklist for Phase 3.` |
...
| 2026-04-02 | P4-T16 | Stabilized text-tool draft handoff by giving each draft its own identity, deferring new draft creation by one main-loop turn after committing an active draft, explicitly dismantling the old `NSTextField` responder/editor before replacement, keeping the overlay panel key during text-to-text handoffs, and routing text-editor-originated clicks through screen-coordinate hit resolution instead of requiring `event.window` to be an `OverlayPanel`, so clicking a new text location mid-edit cleanly commits the old note and starts the next one | `swiftc -frontend -parse Pinnacle/Overlay/OverlayScene.swift Pinnacle/Overlay/OverlayViewModel.swift Pinnacle/Overlay/OverlayRootView.swift Pinnacle/Overlay/AppKitOverlayService.swift Pinnacle/Overlay/DisplayDescriptor.swift PinnacleTests/PinnacleSmokeTests.swift` passed; `xcodebuild` unavailable because `xcode-select` points to CommandLineTools | P3-T09 |
| 2026-04-06 | P4-T17 | Disabled `RemoteViewService` and system text input accessory features (spell checking, text replacement, smart quotes, touch bar character picker) on `OverlayTextField` by configuring `allowsEditingTextAttributes`, `allowsCharacterPickerTouchBarItem`, and implementing `controlTextDidBeginEditing` to disable smart features on the underlying `NSTextView`. This prevents `ViewBridge` errors (e.g. `NSViewBridgeErrorCanceled`) and task name port failures when the floating overlay window isn't a standard active application window. | `swiftc -frontend -parse Pinnacle/Overlay/OverlayScene.swift Pinnacle/Overlay/OverlayViewModel.swift Pinnacle/Overlay/OverlayRootView.swift Pinnacle/Overlay/AppKitOverlayService.swift Pinnacle/Overlay/DisplayDescriptor.swift PinnacleTests/PinnacleSmokeTests.swift` passed | P3-T09 |
