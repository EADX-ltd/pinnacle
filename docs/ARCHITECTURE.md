...
- [ ] Phase 3: Overlay engine (single display) — code complete; `P3-T09` manual radial UX pending
- [x] Phase 4: Tool renderers + undo/redo
- [x] Phase 5: Multi-display support
- [ ] Phase 6: Recording engine integration — code complete; `P6-T06` manual recording lifecycle matrix pending (only validation path for PTS-rebasing freeze fix)
- [ ] Phase 7: Settings UI for shortcuts/colors — code complete; `P7-T05` manual settings flow checklist pending
- [x] Phase 8: Persistence and output management
- [ ] Phase 9: Stabilization, profiling, test pass — `P9-T01/T04/T05` done; `P9-T02` manual matrix and `P9-T03` perf profiling pending
...
| 2026-04-02 | 4 | Updated the default annotation presets so text and highlighter sizes start at `14`, pen/highlighter/text default to yellow, arrow/rectangle/ellipse default to blue, and the eraser cursor glyph renders red | Aligns the initial tool behavior with the newly requested visual defaults while keeping existing per-tool option flows and cursor hotspot behavior unchanged | Validation: `swiftc -frontend -parse Pinnacle/Domain/AppDomainModels.swift Pinnacle/Overlay/OverlayViewModel.swift Pinnacle/Overlay/OverlayPanel.swift PinnacleTests/PinnacleSmokeTests.swift` passes; `xcodebuild test` cannot run here because `xcode-select` points to CommandLineTools. |
| 2026-04-02 | 4 | Replaced the broken tinted-SF-Symbol eraser cursor with dedicated custom artwork and added an explicit overlay activation hook so keyboard tool shortcuts reuse the same radial-selection path as mouse clicks, including exiting pass-through mode | The symbol-based cursor did not render reliably as a red eraser in practice, and shortcut-driven tool selection needed to resume annotation mode with the same UI state transition users get from clicking a radial tool | Validation: `swiftc -frontend -parse` passes; `xcodebuild` remains unavailable here because `xcode-select` points to CommandLineTools. |
| 2026-04-02 | 4 | Stabilized the text-tool edit handoff by assigning each `TextDraft` a unique identity, deferring replacement draft creation until the next main-loop turn after commit, explicitly dismantling the previous `NSTextField` editor/responder when SwiftUI removes it, keeping the overlay panel key during text-to-text clicks, and resolving overlay mouse clicks from screen coordinates instead of assuming the editor click arrives through an `OverlayPanel` window | Clicking a second text location while editing was functionally creating the next draft, but AppKit input analytics and ViewBridge warnings indicated the previous field was being torn down mid-session without a clean responder shutdown, and the replacement click could be dropped because the active text editor was not always reporting events through the overlay panel window | Validation: `swiftc -frontend -parse` passes; `xcodebuild` remains unavailable here because `xcode-select` points to CommandLineTools. |
| 2026-04-06 | 4 | Disabled `RemoteViewService` triggers and smart text features (spell checking, text replacement, smart quotes, touch bar character picker) on the `OverlayTextField` and its underlying `NSTextView` | The system automatically attempts to establish remote view service connections for these accessories on `NSTextField`, which fails with `NSViewBridgeErrorCanceled` and task name port failures when the floating overlay window isn't fully configured as a standard active application window | Validation: `swiftc -frontend -parse` passes; `xcodebuild` remains unavailable here because `xcode-select` points to CommandLineTools. |
| 2026-05-05 | post-5 | Landed P0 review refactors: split `AppStore.reduce()` into per-command-group helpers (`reduceAnnotationCommand` / `reduceRecordingCommand` / `reduceToolCommand`); replaced four decoupled radial bools (`isRadialExpanded`, `isOptionsOpen`, `isPassThroughMode`, `selectedToolForOptions`) with a single `RadialState` enum (`collapsed`, `expanded(selectedToolForOptions:)`, `optionsOpen(ToolKind)`, `passThrough`); extracted `TextEditingViewModel` to own the NSTextField responder lifecycle (encodes the P4-T17 contract as a docblock); added `OverlayService.setErrorHandler` so panel-creation and screen-resolution failures surface in `AppStore.lastErrorMessage` instead of being silently logged | Tracks the prioritized review plan in `.air/plans/review-this-application-how-bubbly-simon.plan.md` (P0 structural debt). The four bool radial state was already producing impossible combinations, the 522-LOC `OverlayViewModel` was the largest file in the codebase, and overlay startup errors were invisible to the user. | Validation: `xcodebuild test -scheme Pinnacle -destination 'platform=macOS'` passes (all `PinnacleSmokeTests` green). Old radial bool getters preserved as computed read-only props for backward compatibility. Also folded in P1-7 (consolidate duplicated `globalPointToLocal` calls in `OverlayRootView`) and P2-8/9 (inline coordinate-space contract on `handleDragChanged`, NSTextField responder contract docblock on `TextEditingViewModel`). P1-5 (`ColorHex` newtype) deferred — touches `ToolConfig` and most `OverlaySceneElement` kinds plus all tests; tracked separately. |
| 2026-05-05 | post-5 | Landed P1-5: introduced `ColorHex` newtype (`Equatable, Hashable, Codable, ExpressibleByStringLiteral`) wrapping the raw RGBA hex string. Migrated `ToolConfig.colorHexRGBA`, all five `OverlaySceneElement.Kind` cases, `OverlayTextItem.colorHexRGBA`, the `Color(hexRGBA:)` initializer, and the cycle/swatch palette literals in `AppStore` and `ToolOptionsPanelView` to the new type. | Replaces a stringly-typed RGBA channel with a domain-specific value, preventing accidental cross-pollination with arbitrary strings while keeping migration churn minimal because string literals coerce via `ExpressibleByStringLiteral`. | Validation: `xcodebuild test -scheme Pinnacle -destination 'platform=macOS'` passes (all `PinnacleSmokeTests` green) — every existing test's hex literal call site was preserved untouched. Codable single-value conformance keeps preference JSON wire format identical (raw hex string). |
| 2026-05-05 | post-5 | Completed P2-10 read-only audit of `AppKitOverlayService` `[weak self]` and `removeObserver` paths. Findings: (1) all four Combine `.sink` pipelines, both view-model callbacks, the screen-parameters `NotificationCenter` block, and both `NSEvent` local monitors correctly capture `[weak self]`; (2) `cancellables` Set anchors the Combine subscriptions for the service's lifetime; (3) `screenObserver`, `drawEventMonitor`, and `keyEventMonitor` are removed only inside `stopOverlay()` — there is no `deinit` to mop them up if the service is deallocated while the overlay is running; (4) the screen-parameters and key-event handlers nest a redundant outer `[weak self]` whose `self` is shadowed and unused because the body immediately spawns `Task { @MainActor [weak self] in ... }`. | Confirms the lifetime story is sound for the current singleton-style `AppContainer.live` ownership model. Noted residual risks (no-deinit cleanup, double-weak nesting) are documentation-grade rather than active bugs since the service is a long-lived singleton in practice. | No code changes. Items deferred for future cleanup: add a defensive `deinit` calling `removeMouseEventMonitor`, `removeKeyEventMonitor`, and `NotificationCenter.default.removeObserver` if `screenObserver` is still bound; collapse the redundant outer `[weak self]` capture in the two `Task`-wrapping closures. |
| 2026-05-08 | 9 (partial) | Phase 9 code-reachable closure. P9-T01: 73 unit tests green via `xcodebuild test`. P9-T04: defect log captured in `docs/RELEASE_READINESS.md` (all P0/P1 from the audit pass resolved; reviewer P2s landed; 4 lower-severity items deferred with rationale). P9-T05: `docs/RELEASE_READINESS.md` published with **Conditional GO** recommendation — code complete; manual matrix (P3-T09 + P6-T06 + P7-T05) and performance profiling (P9-T03) required before tagging v1.0. P9-T02 (manual matrix) and P9-T03 (perf) explicitly require GUI session and are out of scope for code-only completion. | A clean stopping point for autonomous code work; the remaining gates are inherently human-in-the-loop. | Validation: 73/73 unit tests passing. The pause/resume freeze-artifact fix remains validatable only via P6-T06 (real recording walk). |
| 2026-05-08 | 8 | Phase 8 persistence + recording output management. P8-T01 confirmed `UserDefaultsPreferencesService` already implements the repository (every preference key round-trips through it). P8-T02: `preferencesSchemaVersionKey` (current=1) + `migratePreferencesIfNeeded(using:)` invoked from `AppStore.init` ahead of any prefs reads; future schema changes branch on the stored version here. P8-T03: extended `RecordingService` with `outputDirectory: URL { get set }`; `ScreenCaptureKitRecordingService.defaultOutputDirectory()` exposed as a class method; `OutputPanelView` provides folder picker via `NSOpenPanel.beginSheetModal(for:)` plus Reveal in Finder and Reset to Default; `setOutputDirectory(_:)` validates writability via probe write-and-delete and throws `OutputDirectoryError`. P8-T04: `uniqueFileURL(in:baseName:ext:)` appends `-1`, `-2`, ..., `-99`, then a UUID suffix so an existing file is never overwritten. P8-T05 deferred (labeled `Optional`). P8-T06: persistence tests cover all of the above plus schema-version stamp on first init. | Recording output is the user's primary artifact; collision-free naming and a writable location were both required to avoid silent data loss. Schema versioning costs near-zero now and unblocks future migrations. | Validation: 73/73 unit tests passing. |
| 2026-05-08 | 7 | Phase 7 Settings UI complete — 5-tab `SettingsView` (Shortcuts, Tools, Radial, Output, Permissions). Each tab is its own SwiftUI view backed by `AppStore` API: `updateShortcutBindings(_:) -> Bool` with live conflict feedback (orange banner + per-row highlight), `updateToolConfig(_:for:)` / `updateToolExtendedOptions(_:for:)` with palette swatches + `ColorPicker` + sliders + segmented extended options, `setRadialControlEnabled(_:)` + `setRadialDefaultPosition(_:)` (new `RadialPosition` domain enum), `setOutputDirectory(_:)` (probe write-and-delete validation, throws `OutputDirectoryError`), `permissionStatus()` async + `requestScreenRecordingAccess()` with deep link to System Settings. Each editor has a `Reset to Defaults` action guarded by destructive `.alert`. Closed the UX gap from the audit's conflict-preserve fix — users with persisted shortcut conflicts can now resolve them. | Phase 7 was the last user-facing surface needed before stabilization; it also unblocks the new audit-introduced affordance of letting users repair persisted conflicts in-app rather than via plist editing. | Validation: 73/73 unit tests passing — covers shortcut update + conflict + reset, tool style round-trip + persistence + reset, radial prefs propagation, output dir set/reset/reject-non-directory + `uniqueFileURL` collision avoidance, permission service forwarding. Manual P7-T05 checklist (9 sections) authored in `docs/MANUAL_TEST_GUIDE.md`; awaiting GUI session. Reviewer P2 follow-ups (probe-write check, `beginSheetModal` for `NSOpenPanel`) landed in the same pass. |
| 2026-05-08 | 6 (audit) | `/feature-dev fix all` audit pass remediated 14 P0/P1 findings: recording (PTS-rebasing pause/resume, `awaitFinalization()` drained from `PinnacleAppDelegate.applicationShouldTerminate` via `terminateLater`/`reply(toApplicationShouldTerminate:)`, startup-Task cancel/await on stop, `FinishOutcome` surface for empty-file stop), overlay (shared `OverlayGeometry` constants so dead-zone matches hit-test tiers, multi-display panel retention via `NSScreen.screens` iteration, `HUDView` wired into `OverlayRootView`), AppStore (paused-error routing using captured `modeBeforePause`, paused annotation toggle implemented, conflict resolver no longer overwrites stored bindings), services (Carbon hotkey rollback on partial failure, `SystemPermissionService` distinguishes `.denied` via UserDefaults `hasRequested` flag), hygiene (`StubServices.swift` moved to `PinnacleTests/`). | The Phase 6 implementation passed compile + smoke tests but hadn't been audited end-to-end for correctness; the user directive `/feature-dev fix all` triggered a full sweep with three parallel reviewer agents and an advisor checkpoint that caught an `endPause` second-attempt bug before commit. | Validation: 51/51 unit tests passing at audit close; manual matrix `P6-T06` (record/pause/resume/stop scenario) remains the only validation path for the PTS-rebasing freeze fix and is still pending. |
| 2026-05-05 | 6 | Phase 6 kickoff (`P6-T01`..`P6-T05`, `P6-T07`): extended `RecordingService` protocol with `isPaused`, `outputURL`, mutable `capturesSystemAudio`, `pauseRecording`/`resumeRecording`, and `setErrorHandler`; created `ScreenCaptureKitRecordingService` (SCStream + SCStreamConfiguration + SCStreamOutput/Delegate adapter, `~/Movies/Pinnacle/Pinnacle-yyyyMMdd-HHmmss.mp4` output, 1/30 frame interval, queueDepth 8, BGRA + sRGB, `showsCursor=true`, audio capture gated on macOS 13+); added thread-safe `AVAssetWriterPipeline` (NSLock-guarded video/audio appends, H.264 6 Mbps @ 1080p/30 + AAC 48 kHz/2ch/128 kbps when audio toggled, first-sample `startSession`); added real `SystemPermissionService` using `CGPreflightScreenCaptureAccess`/`CGRequestScreenCaptureAccess`; rewired `AppContainer.live` to use the new services; added `DisplayDescriptor.underMouse()` for session-display selection; in `AppStore`: added `@Published var capturesSystemAudio` syncing into the service, wired `setErrorHandler` to surface failures and reset session state, and replaced TODOs with real `pauseRecording`/`resumeRecording` calls; extended `SpyRecordingService` test double and added six new tests covering pause/resume routing, idempotent start/stop, error-handler session reset, and audio toggle propagation. | Phase 6 needs a working capture pipeline before Phase 7 saves/exports can be designed; defaults agreed: H.264 1080p@30 6 Mbps with optional system audio toggleable by the presenter, sync TCC preflight + `requestScreenRecordingAccess()` fallback that throws `RecordingError.permissionRequired`, single-display capture using the same mouse-display contract as Phase 5 overlay sessions. | Validation: full-target `swiftc -typecheck` passes clean (only unrelated `#Preview` macro plugin warnings); `xcodebuild test` and the manual GUI scenario list (P6-T06) remain deferred to a full macOS/Xcode runtime, parallel with the still-open P3-T09 manual checklist. Residual risk: real capture quality, file finalization on abrupt pause/stop transitions, and `Movies/Pinnacle` directory creation behavior all require live macOS verification before P6-T06 can close. |
